...
-- -----------------------------------------------------
DROP TABLE IF EXISTS `bd_mecanica`.`clientes` ;
...
CREATE TABLE IF NOT EXISTS `bd_mecanica`.`clientes` (
  ...
  `cep` INT(8) UNSIGNED NULL COMMENT 'Informar o CEP do CLiente.',
  PRIMARY KEY (`id`),
...
-- -----------------------------------------------------
DROP TABLE IF EXISTS `bd_mecanica`.`carros` ;
...
CREATE TABLE IF NOT EXISTS `bd_mecanica`.`carros` (
  ...
  `ano_modelo` DATE NOT NULL COMMENT 'Será inserido o ano do modelo:\nEX: 2013',
  `ano_fabricado` DATE NOT NULL COMMENT 'Será inserido o ano fabricado do carro:\nEX: 2012',
...
-- -----------------------------------------------------
DROP TABLE IF EXISTS `bd_mecanica`.`orcamentos` ;
...
CREATE TABLE IF NOT EXISTS `bd_mecanica`.`orcamentos` (
  ...
  `clientes_id` INT UNSIGNED ZEROFILL NOT NULL COMMENT 'ID do cliente que está sendo passado o orçamento',
  `carros_id` INT UNSIGNED ZEROFILL NOT NULL COMMENT 'ID do carro que está sendo passado o orçamento.',
...
-- -----------------------------------------------------
DROP TABLE IF EXISTS `bd_mecanica`.`cargos` ;
...
-- -----------------------------------------------------
DROP TABLE IF EXISTS `bd_mecanica`.`funcionarios` ;

CREATE TABLE IF NOT EXISTS `bd_mecanica`.`funcionarios` (
  `matricula` TINYINT ZEROFILL UNSIGNED NOT NULL AUTO_INCREMENT,
...
-- -----------------------------------------------------
DROP TABLE IF EXISTS `bd_mecanica`.`ordens_de_servicos` ;
...
CREATE TABLE IF NOT EXISTS `bd_mecanica`.`ordens_de_servicos` (
  ...
  `orcamentos_id` INT UNSIGNED ZEROFILL NOT NULL COMMENT 'Informar o ID do ORçamento',
  `funcionarios_matricula` TINYINT ZEROFILL UNSIGNED NOT NULL COMMENT 'Informar o ID do funcionario que fará o serviço.',
  `desconto` DECIMAL(6,2) NULL COMMENT 'Informar quando disponibilizar um desconto.',
  `valor_final` DECIMAL(6,2) ZEROFILL NOT NULL COMMENT 'Inserir o valor final.\nEX: Caso tenha que fazer hora extra, ou se for dado algum desconto para o cliente.',
  PRIMARY KEY (`id`),
  INDEX `fk_ordem_de_servico_orcamentos1_idx` (`orcamentos_id` ASC) VISIBLE,
  INDEX `fk_ordem_de_servico_funcionarios1_idx` (`funcionarios_matricula` ASC) VISIBLE,
  UNIQUE INDEX `id_UNIQUE` (`id` ASC) VISIBLE,
  CONSTRAINT `fk_ordem_de_servico_orcamentos1`
    FOREIGN KEY (`orcamentos_id`)
    REFERENCES `bd_mecanica`.`orcamentos` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_ordem_de_servico_funcionarios1`
